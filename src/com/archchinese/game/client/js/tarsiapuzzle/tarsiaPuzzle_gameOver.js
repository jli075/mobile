...
tarsiaPuzzle.gameOver = (function() {
	var scene = fn_createScene(tarsiaPuzzle.getGameOverId());
	...
	scene.create = function() {
		...
		//final score
		var totalTime = tarsiaPuzzle.gameScene1.totalTime;
		if (totalTime > 0) {//only show time if it is not aborted by clicking the stop button.
			this.finalTime = fn_formatMilliSeconds(tarsiaPuzzle.gameScene1.totalTime);		
	...
	scene.setGameLeaderboard = function(leaderboard) {		
		fn_showTimeLeaderboard(this, leaderboard, tarsiaPuzzle.gameScene1.totalTime, document);
	}.bind(scene);
