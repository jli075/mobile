...
balloonPop.gameOver = (function() {
	var scene = fn_createScene(balloonPop.getGameOverId());
	...
	scene.create = function() {
		...
		//final score
		var totalTime = balloonPop.gameScene1.totalTime;
		if (totalTime > 0) {//only show time if it is not aborted by clicking the stop button.
			this.finalTime = fn_formatMilliSeconds(balloonPop.gameScene1.totalTime);		
	...
	scene.setGameLeaderboard = function(leaderboard) {		
		fn_showTimeLeaderboard(this, leaderboard, balloonPop.gameScene1.totalTime, document);
	}.bind(scene);
