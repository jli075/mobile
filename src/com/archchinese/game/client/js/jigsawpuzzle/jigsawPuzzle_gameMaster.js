...
//The Module Pattern
jigsawPuzzle.gameMaster = (function(){
	//private section
	function config(){//private function
		...
		conf.scene.push(jigsawPuzzle.gameTitle);
		conf.scene.push(jigsawPuzzle.gameHelp);
		conf.scene.push(jigsawPuzzle.gameOptions);
		conf.scene.push(jigsawPuzzle.gameGlass);
		conf.scene.push(jigsawPuzzle.gameScene1);
		conf.scene.push(jigsawPuzzle.gameOver);
		
		//remove gravity for Drag and Match
		conf.physics.arcade.gravity.y = 0;
		conf.type = fn_getTypeCanvas();
		return conf;
	}
	...
	gameAgent.setMainGameScene(jigsawPuzzle.gameScene1);
	gameAgent.setGameOverScene(jigsawPuzzle.gameOver);
