...
jigsawPuzzle.gameOver = (function() {
	var scene = fn_createScene(jigsawPuzzle.getGameOverId());
	...
	scene.create = function() {
		...
		//final score
		var totalTime = jigsawPuzzle.gameScene1.totalTime;
		if (totalTime > 0) {//only show time if it is not aborted by clicking the stop button.
			this.finalTime = fn_formatMilliSeconds(jigsawPuzzle.gameScene1.totalTime);		
	...
	scene.setGameLeaderboard = function(leaderboard) {		
		fn_showTimeLeaderboard(this, leaderboard, jigsawPuzzle.gameScene1.totalTime, document);
	}.bind(scene);
